//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.0

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:crossly_buyer/api.dart';
import 'package:test/test.dart';

// tests for ListSalesItem
void main() {
  final instance = ListSalesItem();

  group('test ListSalesItem', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // String platform
    test('to test the property `platform`', () async {
      // TODO
    });

    // DateTime cancelledAt
    test('to test the property `cancelledAt`', () async {
      // TODO
    });

    // String orderId
    test('to test the property `orderId`', () async {
      // TODO
    });

    // String listingId
    test('to test the property `listingId`', () async {
      // TODO
    });

    // String inventoryItemId
    test('to test the property `inventoryItemId`', () async {
      // TODO
    });

    // String platformListingId
    test('to test the property `platformListingId`', () async {
      // TODO
    });

    // String platformOrderId
    test('to test the property `platformOrderId`', () async {
      // TODO
    });

    // String costOfGoods
    test('to test the property `costOfGoods`', () async {
      // TODO
    });

    // DateTime deletedAt
    test('to test the property `deletedAt`', () async {
      // TODO
    });

    // String salePrice
    test('to test the property `salePrice`', () async {
      // TODO
    });

    // String platformFee
    test('to test the property `platformFee`', () async {
      // TODO
    });

    // String shippingCost
    test('to test the property `shippingCost`', () async {
      // TODO
    });

    // String netProfit
    test('to test the property `netProfit`', () async {
      // TODO
    });

    // String taxCollected
    test('to test the property `taxCollected`', () async {
      // TODO
    });

    // String shippingCharged
    test('to test the property `shippingCharged`', () async {
      // TODO
    });

    // String feeSource
    test('to test the property `feeSource`', () async {
      // TODO
    });

    // String detectedBy
    test('to test the property `detectedBy`', () async {
      // TODO
    });

    // DateTime detectedAt
    test('to test the property `detectedAt`', () async {
      // TODO
    });

    // List<ListSalesItemDocuments> documents (default value: const [])
    test('to test the property `documents`', () async {
      // TODO
    });

    // String refundedAmount
    test('to test the property `refundedAmount`', () async {
      // TODO
    });


  });

}
