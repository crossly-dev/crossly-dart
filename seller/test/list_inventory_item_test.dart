//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.0

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:crossly/api.dart';
import 'package:test/test.dart';

// tests for ListInventoryItem
void main() {
  final instance = ListInventoryItem();

  group('test ListInventoryItem', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String brand
    test('to test the property `brand`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // String sku
    test('to test the property `sku`', () async {
      // TODO
    });

    // num quantity
    test('to test the property `quantity`', () async {
      // TODO
    });

    // num quantityAvailable
    test('to test the property `quantityAvailable`', () async {
      // TODO
    });

    // num quantityReserved
    test('to test the property `quantityReserved`', () async {
      // TODO
    });

    // num quantitySold
    test('to test the property `quantitySold`', () async {
      // TODO
    });

    // String originalPrice
    test('to test the property `originalPrice`', () async {
      // TODO
    });

    // String condition
    test('to test the property `condition`', () async {
      // TODO
    });

    // String size
    test('to test the property `size`', () async {
      // TODO
    });

    // String material
    test('to test the property `material`', () async {
      // TODO
    });

    // String style
    test('to test the property `style`', () async {
      // TODO
    });

    // String pattern
    test('to test the property `pattern`', () async {
      // TODO
    });

    // String department
    test('to test the property `department`', () async {
      // TODO
    });

    // String gender
    test('to test the property `gender`', () async {
      // TODO
    });

    // String itemType
    test('to test the property `itemType`', () async {
      // TODO
    });

    // String sizeSystem
    test('to test the property `sizeSystem`', () async {
      // TODO
    });

    // List<String> color (default value: const [])
    test('to test the property `color`', () async {
      // TODO
    });

    // String defaultTitle
    test('to test the property `defaultTitle`', () async {
      // TODO
    });

    // List<String> images (default value: const [])
    test('to test the property `images`', () async {
      // TODO
    });

    // String videoUrl
    test('to test the property `videoUrl`', () async {
      // TODO
    });

    // List<String> tags (default value: const [])
    test('to test the property `tags`', () async {
      // TODO
    });

    // List<String> labels (default value: const [])
    test('to test the property `labels`', () async {
      // TODO
    });

    // String notes
    test('to test the property `notes`', () async {
      // TODO
    });

    // String customLabel
    test('to test the property `customLabel`', () async {
      // TODO
    });

    // String warehouseLocation
    test('to test the property `warehouseLocation`', () async {
      // TODO
    });

    // String storageBin
    test('to test the property `storageBin`', () async {
      // TODO
    });

    // String storageNote
    test('to test the property `storageNote`', () async {
      // TODO
    });

    // String marketSkuId
    test('to test the property `marketSkuId`', () async {
      // TODO
    });

    // String marketVariantId
    test('to test the property `marketVariantId`', () async {
      // TODO
    });

    // DateTime firstListedAt
    test('to test the property `firstListedAt`', () async {
      // TODO
    });

    // bool includeTaxInLanded
    test('to test the property `includeTaxInLanded`', () async {
      // TODO
    });

    // num costBasisCents
    test('to test the property `costBasisCents`', () async {
      // TODO
    });

    // num priceFloorCents
    test('to test the property `priceFloorCents`', () async {
      // TODO
    });

    // bool floorIsNet
    test('to test the property `floorIsNet`', () async {
      // TODO
    });

    // DateTime acquiredAt
    test('to test the property `acquiredAt`', () async {
      // TODO
    });

    // String acquiredSource
    test('to test the property `acquiredSource`', () async {
      // TODO
    });

    // String weightLb
    test('to test the property `weightLb`', () async {
      // TODO
    });

    // String weightOz
    test('to test the property `weightOz`', () async {
      // TODO
    });

    // String dimensionLIn
    test('to test the property `dimensionLIn`', () async {
      // TODO
    });

    // String dimensionWIn
    test('to test the property `dimensionWIn`', () async {
      // TODO
    });

    // String dimensionHIn
    test('to test the property `dimensionHIn`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // String source_
    test('to test the property `source_`', () async {
      // TODO
    });

    // ListInventoryItemCategory category
    test('to test the property `category`', () async {
      // TODO
    });


  });

}
